...
const manipulate = () => {
  ...
  for (let i = dayone; i > 0; i--) {
    lit += `<button id="btn" class="inactive">${
      monthlastdate - i + 1
    }</button>`;
  }
  ...
  for (let i = 1; i <= lastdate; i++) {
    ...
    let isToday =
      ...
      year === new Date().getFullYear()
        ? "active"
        : "";
    lit += `<button id="btn" class="${isToday}">${i}</button>`;
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  section.addEventListener("click", function (event) {
    ...
    if (event.target.tagName === "BUTTON") {
      ...
      let currdays = event.target.innerText;
      const appointmentDate = document.getElementById("date");
	  let tempDate = currdays;
	  let pos;
	  
	  if (tempDate[tempDate.length -1] == 1 && tempDate != 11){
		pos = "st";
		
		
	  }else if(tempDate[tempDate.length -1] == 2 && tempDate != 12){
		pos = "nd";
	  }
	  else if(tempDate[tempDate.length -1] == 3 && tempDate != 13){
		pos = "rd";
	  }
	  else{pos = "th";}

      appointmentDate.innerText = `${currdays}${pos} ${months[month]} ${year}`;
    }
  });
});

const appointmentTime = document.getElementById("time");
appointmentTime.innerText = "10:00 AM - 11:00 AM";
